...
public sealed class ShotGunShootEnemyAnimator : MonoBehaviour
{
    ...
    {
        yield return PlayShootEnemy(shellKind, ShotGunFireEffectContext.PlayerShootsEnemy);
    }

    public IEnumerator PlayShootEnemy(ShotGunShellKind shellKind, ShotGunFireEffectContext fireEffectContext)
    {
        if (shotGun == null)
        {
            yield break;
    ...
    private Transform GetFireEffect(ShotGunFireEffectContext fireEffectContext)
    {
        ...
        if (effect == null && TryGetComponent(out ShotGunShootPlayerAnimator playerAnimator))
        {
            effect = playerAnimator.GetFireEffectReference(fireEffectContext);
        }
